...
SELECT DISTINCT ON (osd.seller_id)
    CAST(NULLIF(BTRIM(osd.seller_id, ' ''"'), '') AS UUID) AS seller_id,
    CAST(NULLIF(BTRIM(osd.seller_zip_code_prefix, ' ''"'), '') AS INTEGER) AS seller_zip_code_prefix,
    NULLIF(BTRIM(osd.seller_city, ' ''"'), '') AS seller_city,
    NULLIF(BTRIM(osd.seller_state, ' ''" '), '') AS seller_state
